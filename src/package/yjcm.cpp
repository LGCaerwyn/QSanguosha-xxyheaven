...
void JieyueCard::onEffect(const CardEffectStruct &effect) const
{
    if (!effect.to->isNude()) {
        ...
        if (card != NULL)
            effect.from->addToPile("jieyue", card);
...
public:
    ...
    {
        return (!player->getPile("jieyue").isEmpty() && (pattern == "jink" || pattern == "nullification")) || (pattern == "@@jieyue" && !player->isKongcheng());
    }

    bool isEnabledAtNullification(const ServerPlayer *player) const
    {
        return !player->getPile("jieyue").isEmpty();
    }
...
public:
    ...
    {
        if (player->getPhase() == Player::Start && !player->getPile("jieyue").isEmpty()) {
            player->broadcastSkillInvoke(objectName(), qrand()%2 + 1);
            room->sendCompulsoryTriggerLog(player, objectName());
			
            DummyCard *dummy = new DummyCard(player->getPile("jieyue"));
			CardMoveReason reason(CardMoveReason::S_REASON_EXCHANGE_FROM_PILE, player->objectName(), "jieyue", QString());
            room->obtainCard(player, dummy, reason, true);
            delete dummy;
        } else if (player->getPhase() == Player::Finish && player->canDiscard(player, "h")) {
    ...
    int getEffectIndex(const ServerPlayer *player, const Card *card) const
    {
        ...
        QString tag_name = QString("AudioEffect:yujin-jieyue=1+2");
    ...
    : Package("YJCM")
{
	...
	yujin->addSkill(new DetachEffectSkill("jieyue", "jieyue"));
