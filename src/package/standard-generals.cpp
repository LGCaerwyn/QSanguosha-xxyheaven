...
public:
    ...
    virtual int getDrawNum(ServerPlayer *zhangliao, int n) const
    {
        ...
        if (num > 0) {
			...
			room->setPlayerMark(zhangliao, "tuxi_target_num", 0);
            int count = 0;
            if (room->askForUseCard(zhangliao, "@@tuxi", "@tuxi-card:::" + QString::number(num))) {
                count = zhangliao->getMark("tuxi_target_num");
				room->setPlayerMark(zhangliao, "tuxi_target_num", 0);
            }
...
class QingjianAllot : public ViewAsSkill
{
public:
    QingjianAllot() : ViewAsSkill("qingjian_allot")
    {
        expand_pile = "qingjian";
		response_pattern = "@@qingjian_allot!";
    }

    virtual bool viewFilter(const QList<const Card *> &, const Card *to_select) const
    {
        QList<int> qingjian_list = StringList2IntList(Self->property("qingjian_give").toString().split("+"));
        return qingjian_list.contains(to_select->getEffectiveId());
    }

    virtual const Card *viewAs(const QList<const Card *> &cards) const
    {
        if (cards.isEmpty()) return NULL;
        QingjianAllotCard *qingjian_card = new QingjianAllotCard;
        qingjian_card->addSubcards(cards);
        return qingjian_card;
    }
};
...
public:
    ...
    {
        QList<int> qingjian_list = StringList2IntList(Self->property("qingjian").toString().split("+"));
        return qingjian_list.contains(to_select->getEffectiveId());
    }
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
		if (triggerEvent == CardsMoveOneTime && TriggerSkill::triggerable(player)) {
            ...
            if (!room->getTag("FirstRound").toBool() && player->getPhase() != Player::Draw && move.to == player && move.to_place == Player::PlaceHand) {
                ...
                if (ids.isEmpty())
                    return false;
			    room->setPlayerProperty(player, "qingjian", IntList2StringList(ids).join("+"));
			    const Card *card = room->askForCard(player, "@@qingjian", "@qingjian-put", data, Card::MethodNone);
			    room->setPlayerProperty(player, "qingjian", QString());
			    if (card){
			    	LogMessage log;
                    log.type = "#InvokeSkill";
                    log.arg = objectName();
			    	...
			    	player->addToPile("qingjian", card, false);
		    	}
            }
		} else if (triggerEvent == EventPhaseStart && player->getPhase() == Player::NotActive) {
			foreach (ServerPlayer *xiahou, room->getAllPlayers()) {
                if (xiahou->getPile("qingjian").length() > 0 && TriggerSkill::triggerable(xiahou)) {
			        room->sendCompulsoryTriggerLog(player, objectName());
                    player->broadcastSkillInvoke(objectName());
				    QList<int> ids = xiahou->getPile("qingjian");
					room->setPlayerProperty(xiahou, "qingjian_give", IntList2StringList(ids).join("+"));
					do {
						const Card *use = room->askForUseCard(xiahou, "@@qingjian_allot!", "@qingjian-give", -1, Card::MethodNone);
			            ids = StringList2IntList(xiahou->property("qingjian_give").toString().split("+"));
                        if (use == NULL){
							QingjianAllotCard *qingjian_card = new QingjianAllotCard;
                            qingjian_card->addSubcards(ids);
                            QList<ServerPlayer *> targets;
                            targets << room->getOtherPlayers(xiahou).first();
                            qingjian_card->use(room, xiahou, targets);
                            delete qingjian_card;
							break;
						}
					} while (!ids.isEmpty() && xiahou->isAlive());
					room->setPlayerProperty(xiahou, "qingjian_give", QString());
					...
					foreach (ServerPlayer *p, room->getAllPlayers()) {
						...
						CardMoveReason reason(CardMoveReason::S_REASON_EXCHANGE_FROM_PILE, p->objectName(), "qingjian", QString());
...
public:
    ...
    virtual void onDamaged(ServerPlayer *simayi, const DamageStruct &damage) const
    {
        ...
        for (int i = 0; i < damage.damage; i++) {
            if (from && !from->isNude() && room->askForSkillInvoke(simayi, "fankui", QVariant::fromValue(from))) {
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        const Card *card = room->askForCard(player, "..", prompt, data, Card::MethodResponse, judge->who, true, "guicai");
        if (card) {
			if (judge->who)
...
public:
    ...
    virtual bool isEnabledAtPlay(const Player *player) const
    {
        ...
        foreach (const Player *p, player->getAliveSiblings()) {
            if (!rende_prop.contains(p->objectName())) return true;
        }
        return false;
    }
...
public:
    ...
    {
		if (triggerEvent == EventPhaseChanging && player->getMark("rende") > 0) {
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *liubei, QVariant &data) const
    {
		...
		if (target)
			room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, liubei->objectName(), target->objectName());
...
public:
    ...
    {
        return (player->getMark("#skill_invalidity") == 0 && player->getMark("#yijue") == 0 
				&& player->getMark("#tieji") == 0) || skill->getFrequency(player) == Skill::Compulsory;
    }
};
...
public:
    ...
    virtual const Card *viewAs() const
    {
        ...
        skill_card->setUserString(Self->tag["jianyan"].toString());
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *, ServerPlayer *lvmeng, QVariant &data) const
    {
        if (triggerEvent == EventPhaseChanging) {
...
public:
    ...
    {
        events << TargetSpecified << TargetConfirmed << CardFinished;
        frequency = Compulsory;
    }
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == TargetSpecified && TriggerSkill::triggerable(player)) {
			...
			CardUseStruct use = data.value<CardUseStruct>();
			if (use.card->isKindOf("Duel") && use.to.contains(player)) {
                player->broadcastSkillInvoke(objectName());
                room->sendCompulsoryTriggerLog(player, objectName());
                player->tag["Wushuang_" + use.card->toString()] = QStringList(use.from->objectName());
            }
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (damage.to->isAlive() && player != damage.to && !damage.to->hasFlag("Global_DebutFlag") && !damage.to->isNude()
            && damage.card && damage.card->isKindOf("Slash") && damage.to->askForSkillInvoke("liyu_invoke", "prompt:" + player->objectName())) {
            ...
            if (targets.isEmpty()) {
                ...
                ServerPlayer *target = room->askForPlayerChosen(damage.to, targets, objectName(), "@liyu:" + player->objectName());
                if (target) {
...
public:
    ...
    {
        if (lidian->getPhase() != Player::Draw) return false;
    ...
    virtual bool isProhibited(const Player *from, const Player *to, const Card *card, const QList<const Player *> &) const
    {
        if (card->isKindOf("Slash")) {
            ...
            if (card->isVirtualCard()) {
                ...
                QList<int> extracost = card->getCostcards();
                if (from->getWeapon() && (subcards.contains(from->getWeapon()->getId()) || extracost.contains(from->getWeapon()->getId()))) {
                ...
                if (from->getOffensiveHorse() && (subcards.contains(from->getOffensiveHorse()->getId()) || extracost.contains(from->getOffensiveHorse()->getId())))
                    rangefix += 1;
            }
            // find yuanshu
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == Damage && TriggerSkill::triggerable(player)) {
            ...
            if (damage.to->isAlive() && !damage.to->hasFlag("Global_DebutFlag")
                ...
                && player->canDiscard(damage.to, "e") && room->askForSkillInvoke(player, objectName(), QVariant::fromValue(damage.to))) {
                player->broadcastSkillInvoke(objectName());
            ...
            if (move.reason.m_skillName == objectName() && move.reason.m_playerId == player->objectName()
                && move.active_ids.length() == 1) {
                const Card *card = Sanguosha->getCard(move.active_ids.first());
                if (card->isKindOf("Horse") && room->getCardPlace(card->getEffectiveId()) == Player::DiscardPile) {
                    room->obtainCard(player, card);
					move.active_ids.clear();
...
void StandardPackage::addGenerals()
{
	...
	xiahoudun->addSkill(new DetachEffectSkill("qingjian", "qingjian"));
	related_skills.insertMulti("qingjian", "#qingjian-clear");
    ...
    xushu->addSkill(new ZhuhaiRecord);
	...
	addMetaObject<QingjianAllotCard>();

    skills << new Xiaoxi << new NonCompulsoryInvalidity << new Jianyan << new GlobalFakeMoveSkill << new QingjianAllot << new RendeBasic;
}
