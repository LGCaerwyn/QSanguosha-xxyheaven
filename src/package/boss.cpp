...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == EventPhaseChanging) {
			...
			if (player->hasFlag("MengTaiSkipPlay") && !player->isSkipped(Player::Discard)) {
				room->sendCompulsoryTriggerLog(player, objectName());
                player->broadcastSkillInvoke(objectName());
			...
			if (player->hasFlag("MengTaiSkipDraw")) {
				room->sendCompulsoryTriggerLog(player, objectName());
                player->broadcastSkillInvoke(objectName());
...
class MengtaiRecord : public TriggerSkill
{
public:
    MengtaiRecord() : TriggerSkill("#mengtai-record")
    {
    ...
    virtual bool trigger(TriggerEvent, Room *, ServerPlayer *player, QVariant &data) const
    {
		...
		if (player->isSkipped(change.to)) {
			switch (change.to) {
            case Player::Draw: player->setFlags("MengTaiSkipDraw"); break;
            case Player::Play: player->setFlags("MengTaiSkipPlay"); break;
			default:
			    break;
            }
		}
...
public:
    ...
    bool isProhibited(const Player *from, const Player *to, const Card *card, const QList<const Player *> & /* = QList<const Player *>() */) const
    {
        ...
        return from->hasSkill(objectName()) && from != to && from->getHp() <= 6;
    }
};
...
private:
    static void BossChangeState(ServerPlayer *boss)
    {
        ...
        case 4: last_state = "-bossbaonu|-bossshouyi"; break;
	    default:
			break;
        }
    ...
    : Package("BossMode")
{
	...
	nian->addSkill(new MengtaiRecord);
	related_skills.insertMulti("bossmengtai", "#mengtai-record");
	nian->addSkill(new BossChange);
	related_skills.insertMulti("bossbaonu", "#baonu-target");

	skills << new BossDongmian << new BossRuizhi << new RuizhiSelect << new BossJingjue << new BossRenxing << new BossBaonu << new BaonuTarget << new BossShouyi;
}
