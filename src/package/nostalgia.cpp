...
NosAnxuCard::NosAnxuCard()
{
}

bool NosAnxuCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const
{
    if (to_select == Self)
        return false;
    if (targets.isEmpty())
...
bool NosAnxuCard::targetsFeasible(const QList<const Player *> &targets, const Player *) const
{
    return targets.length() == 2;
}

void NosAnxuCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
    ...
    int id = room->askForCardChosen(from, to, "h", "nosanxu");
    const Card *cd = Sanguosha->getCard(id);
    from->obtainCard(cd, true);
    if (cd->getSuit() != Card::Spade)
        source->drawCards(1, "nosanxu");
}

class NosAnxu : public ZeroCardViewAsSkill
{
public:
    NosAnxu() : ZeroCardViewAsSkill("nosanxu")
    {
    }

    virtual const Card *viewAs() const
    {
        return new NosAnxuCard;
    }

    virtual bool isEnabledAtPlay(const Player *player) const
    {
        return !player->hasUsed("NosAnxuCard");
    }
};
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (card) {
            if (player->hasInnateSkill("nosguicai") || !player->hasSkill("jilve"))
                player->broadcastSkillInvoke(objectName());
            else
...
{
	General *nos_bulianshi = new General(this, "nos_bulianshi", "wu", 3, false); // YJ 101
    nos_bulianshi->addSkill(new NosAnxu);
    nos_bulianshi->addSkill("zhuiyi");
    ...
    General *nos_liubiao = new General(this, "nos_liubiao", "qun"); // YJ 108
    nos_liubiao->addSkill(new NosZishou);
    nos_liubiao->addSkill("zongshi");
