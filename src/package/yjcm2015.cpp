...
bool ZhenshanCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const
{
	...
	if (!card->targetFilter(card_targets, to_exchange, Self) || Self->isProhibited(to_exchange, card, card_targets))
		return false;

	return can_select || can_exchange;
}
...
const Card *ZhenshanCard::validate(CardUseStruct &card_use) const
{
	...
	ServerPlayer *to_exchange = card_use.to.takeLast();
...
const Card *ZhenshanCard::validateInResponse(ServerPlayer *quancong, QList<ServerPlayer *> &targets) const
{
    Room *room = quancong->getRoom();
	ServerPlayer *to_exchange = targets.first();
...
public:
    ...
    bool isEnabledAtResponse(const Player *player, const QString &pattern) const
    {
        if (!canExchange(player)) return false;
...
public:
    ...
    bool buttonEnabled(const QString &button_name) const
    {
        ...
        if (card->isKindOf("Slash"))
            classname = "Slash";
        if (Self->getMark("Huomo_" + classname) > 0)
            return false;
