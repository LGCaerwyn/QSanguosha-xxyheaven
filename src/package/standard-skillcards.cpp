...
void YijueCard::use(Room *room, ServerPlayer *guanyu, QList<ServerPlayer *> &targets) const
{
    ...
    if (success) {
        ...
        if (room->askForSkillInvoke(guanyu, "yijue_recover", "prompt:" + target->objectName()))
...
{
    return targets.length() < Self->getMark("tuxi") && to_select->getHandcardNum() >= Self->getHandcardNum() && to_select != Self && !to_select->isKongcheng();
}

void TuxiCard::onEffect(const CardEffectStruct &effect) const
{
    Room *room = effect.from->getRoom();
	room->addPlayerMark(effect.from, "tuxi_target_num");
...
void JianyanCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    ...
    while (true) {
        if (room->getDrawPile().isEmpty()){
			bool stop_jianyan = true;
			foreach (int id, room->getDiscardPile()) {
                const Card *c = Sanguosha->getCard(id);
				if (Sanguosha->matchExpPattern(pattern, NULL, c)) {
					stop_jianyan = false;
					break;
				}
            }
...
QingjianAllotCard::QingjianAllotCard()
{
    will_throw = false;
	mute = true;
    handling_method = Card::MethodNone;
...
void QingjianAllotCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
	ServerPlayer *target = targets.first();
    QList<int> rende_list = StringList2IntList(source->property("qingjian_give").toString().split("+"));
	foreach (int id, subcards) {
	    rende_list.removeOne(id);
	}
    room->setPlayerProperty(source, "qingjian_give", IntList2StringList(rende_list).join("+"));
