...
public:
    ...
    virtual void onDamaged(ServerPlayer *caopi, const DamageStruct &) const
    {
        ...
        ServerPlayer *to = room->askForPlayerChosen(caopi, room->getOtherPlayers(caopi), objectName(),
            "@fangzhu-invoke:::" + QString::number(caopi->getLostHp()), caopi->getMark("JilveEvent") != int(Damaged), true);
        if (to) {
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == TargetSpecified) {
            ...
            if (use.card->isKindOf("SavageAssault")) {
                ...
                if (menghuo && menghuo != player) {
                    menghuo->broadcastSkillInvoke(objectName(), 1);
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *zhurong, QVariant &data) const
    {
        ...
        if (damage.card && damage.card->isKindOf("Slash") && !zhurong->isKongcheng()
            ...
            && room->askForSkillInvoke(zhurong, objectName(), QVariant::fromValue(target))) {
...
int Yinghun::getYinghunNum(ServerPlayer *sunjian) const
{
...
bool Yinghun::onPhaseChange(ServerPlayer *sunjian) const
{
	...
	if (sunjian->getPhase() != Player::Start || !sunjian->isWounded()) return false;
