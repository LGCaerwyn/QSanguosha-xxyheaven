...
void Chengxiang::onDamaged(ServerPlayer *target, const DamageStruct &damage) const
{
    ...
    AskForMoveCardsStruct result = room->askForMoveCards(target, card_ids, QList<int>(), true, objectName(), "sum" + QString::number(total_point), 1, 0, false, true);
            ...
            if (damage.to->getHp() == 1) {
                foreach (ServerPlayer *p, room->getOtherPlayers(damage.to)) {
                    if (TriggerSkill::triggerable(p) && p->canDiscard(p, "he")) {
                        if (room->askForCard(p, ".Equip", "@renxin-card:" + damage.to->objectName(), data, objectName(), damage.to)) {
                            LogMessage log;
                            log.type = "#Renxin";
                            log.from = damage.to;
                            log.arg = objectName();
