...
void Card::onUse(Room *room, const CardUseStruct &use) const
{
    ...
    if (card_use.card->getTypeId() == TypeSkill) {
        ...
        thread->trigger(CardExtraCost, room, card_use.from, data);
    }
...
void Card::use(Room *room, ServerPlayer *source, const Card *target) const
{
    QStringList nullified_list = tag["CardUseNullifiedList"].toStringList();
    bool all_nullified = nullified_list.contains("_ALL_TARGETS");
    ...
    effect.card = this;
    effect.from = source;
    effect.to_card = target;
    effect.multiple = false;
    effect.nullified = all_nullified;
