    ...
    bool isRetrial, const QString &skill_name, bool isProvision)
{
        ...
        if ((method == Card::MethodUse || method == Card::MethodResponse) && !isRetrial) {
            if (card->hasFlag("isProvide")) {
			    ...
			    if ((method == Card::MethodUse && card->isKindOf("Jink")) || method == Card::MethodResponse) {
			        setCardEmotion(player, card);//播放动画时机前移
			    }
                player->broadcastSkillInvoke(card);
			}
    ...
    if (card) {
        ...
        if ((method == Card::MethodUse || method == Card::MethodResponse) && !isRetrial) {
            ...
            if (effect.card)
                resp.m_data = QVariant::fromValue(effect.card);
            ...
            thread->trigger(CardExtraCost, this, player, _data);
    ...
    bool isSecondaryHero, bool sendLog)
{
    ...
    if (isSecondaryHero) {
        player->setDeputySkinId(0);
        changePlayerGeneral2(player, new_general);
    } else {
        player->setHeadSkinId(0);
...
void Room::assignGeneralsForPlayers(const QList<ServerPlayer *> &to_assign)
{
    ...
    foreach (ServerPlayer *player, m_players) {
        if (player->getGeneral())
            existed << Sanguosha->getMainGeneral(player->getGeneralName());
        if (player->getGeneral2())
            existed << Sanguosha->getMainGeneral(player->getGeneral2Name());
    }
    ...
    if (card->isVirtualCard()) {
        ...
        if (move.card_ids.size() == 0) {
            if ((((reason.m_reason & CardMoveReason::S_MASK_BASIC_REASON) == CardMoveReason::S_REASON_USE) ||
                    reason.m_reason == CardMoveReason::S_REASON_RESPONSE) && card->getCostcards().isEmpty()) {
...
void Room::swapCards(ServerPlayer *first, ServerPlayer *second, const QString &flags, const QString &skill_name)
{
	...
	try {
        ...
        QList<int> pile1 = first->getPile("wooden_ox"), pile2 = second->getPile("wooden_ox");
        ...
        CardMoveReason reason1(CardMoveReason::S_REASON_SECRETLY_PUT, first->objectName(), second->objectName(), skill_name, QString());
        CardMoveReason reason2(CardMoveReason::S_REASON_SECRETLY_PUT, second->objectName(), first->objectName(), skill_name, QString());
        ...
        if (flags.contains(equip_flag)) {
            CardsMoveStruct move3(pile1, NULL, Player::PlaceTable, reason1);
            CardsMoveStruct move4(pile2, NULL, Player::PlaceTable, reason2);
            move_to_table.push_back(move4);
            move_to_table.push_back(move3);
        }
		if (!move_to_table.isEmpty()) {
			moveCardsAtomic(move_to_table, false);

            CardMoveReason reason3(CardMoveReason::S_REASON_SWAP, first->objectName(), second->objectName(), skill_name, QString());
            CardMoveReason reason4(CardMoveReason::S_REASON_SWAP, second->objectName(), first->objectName(), skill_name, QString());
			...
			if (flags.contains(handcard_flag)) {
                ...
                CardsMoveStruct move1(getCardIdsOnTable(hands1), second, Player::PlaceHand, reason3);
                CardsMoveStruct move2(getCardIdsOnTable(hands2), first, Player::PlaceHand, reason4);
			...
			if (flags.contains(equip_flag)) {
                ...
                p_list << first << second;
                CardsMoveStruct move1(getCardIdsOnTable(equips1), second, Player::PlaceEquip, reason3);
                CardsMoveStruct move2(getCardIdsOnTable(equips2), first, Player::PlaceEquip, reason4);
				back_move.push_back(move2);
				back_move.push_back(move1);
                second->setCardsToPile("wooden_ox", pile1, false, p_list);
                CardsMoveStruct move3;
                move3.card_ids = pile1;
                move3.to = second;
                move3.to_place = Player::PlaceSpecial;
                move3.is_open_pile = false;
                first->setCardsToPile("wooden_ox", pile2, false, p_list);
                CardsMoveStruct move4;
                move4.card_ids = pile2;
                move4.to = first;
                move4.to_place = Player::PlaceSpecial;
                move4.is_open_pile = false;
                back_move.push_back(move4);
...
QList<CardsMoveOneTimeStruct> Room::_mergeMoves(QList<CardsMoveStruct> cards_moves)
{
    ...
    foreach (_MoveMergeClassifier cls, moveMap.keys()) {
        ...
        foreach (CardsMoveStruct move, moveMap[cls]) {
			...
			moveOneTime.active_ids = moveOneTime.card_ids;
...
void Room::moveCardsAtomic(QList<CardsMoveStruct> cards_moves, bool forceMoveVisible)
{
    //for wooden_ox
    foreach (CardsMoveStruct cards_move, cards_moves) {
        if (cards_move.reason.m_reason == CardMoveReason::S_REASON_SECRETLY_PUT) continue;
        foreach (int id, cards_move.card_ids) {
            const Card *card = Sanguosha->getEngineCard(id);
            if (card->objectName() == "wooden_ox") {
                ServerPlayer *move_from = getCardOwner(id);
                if (move_from != NULL) {
                    QList<int> pile = move_from->getPile("wooden_ox");
                    if (pile.isEmpty()) break;
                    ServerPlayer *move_to = (ServerPlayer *)cards_move.to;
                    if (move_to != NULL && cards_move.to_place == Player::PlaceEquip) {
                        QList<ServerPlayer *> p_list;
                        p_list << move_to;
                        move_to->setCardsToPile("wooden_ox", pile, false, p_list);
                        CardsMoveStruct new_move;
                        new_move.card_ids = pile;
                        new_move.to = move_to;
                        new_move.to_place = Player::PlaceSpecial;
                        new_move.is_open_pile = false;
                        cards_moves.append(new_move);
                    } else {
                        CardsMoveStruct new_move;
                        new_move.card_ids = pile;
                        new_move.to_place = Player::DiscardPile;
                        CardMoveReason reason(CardMoveReason::S_REASON_REMOVE_FROM_PILE, move_from->objectName());
                        new_move.reason = reason;
                        cards_moves.append(new_move);
                    }
                }
            }
...
bool Room::broadcastSkillInvoke(const QString &skill_name, const ServerPlayer *player, const QList<int> &types)
{
    ...
    if (skill != NULL) {
		...
		if (player != NULL) {
			...
			if (general_name.isEmpty()) {
				...
				if (player->hasSkill("huashen", true) && huashen_skill == skill_name) {
    ...
    bool optional, bool include_equip, const QString &prompt, const QString &pattern)
{
    ...
    if (!optional) {
        ...
        if (card_num <= min_num) {
            if (card_num > 0) {
                if (reason == "gamerule") {
                    CardMoveReason mreason(CardMoveReason::S_REASON_RULEDISCARD, player->objectName(), QString(), dummy->getSkillName(), reason);
                    throwCard(dummy, mreason, player);
                } else {
...
void Room::sendCompulsoryTriggerLog(ServerPlayer *player, const QString &skill_name, bool notify_skill)
{
	...
	const Skill *skill = Sanguosha->getSkill(skill_name);
	if (skill && player->hasEquipSkill(skill_name))
