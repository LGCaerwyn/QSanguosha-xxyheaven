    ...
    : main_window(main_window), game_started(false), m_tableBgPixmap(1, 1), m_tableBgPixmapOrig(1, 1)
{
    ...
    connect(ClientInstance, SIGNAL(pile_shown(QString, QList<int>, QString)), this, SLOT(showPile(QString, QList<int>, QString)));
...
void RoomScene::handleGameEvent(const QVariant &args)
{
    ...
    case S_GAME_EVENT_SKILL_INVOKED: {
        ...
        ClientPlayer *player = ClientInstance->getPlayer(player_name);
        if (player && (player->hasSkill(skill_name) || player->hasEquipSkill(skill_name)) && player != Self) {
            PlayerCardContainer *container = (PlayerCardContainer *)_getGenericCardContainer(Player::PlaceHand, player);
            Photo *photo = qobject_cast<Photo *>(container);
...
void RoomScene::clearDashboardBox(const QString &skill_name)
{
    SelectBox *select = select_items[skill_name];
    if (select != NULL)
        select->clear();
...
void RoomScene::addSkillButton(const Skill *skill)
{
    ...
    if (select_options != "" && !m_replayControl) {
        if (select_options.startsWith("guhuo_")) {
            ...
            SelectBox *select = new SelectBox(btn->getSkill()->objectName(), select_options.split("+"));
            select->setParent(this);
            addItem(select);
            select->hide();
            select->setZValue(30001);
            select_items[select->getSkillName()] = select;
            connect(btn, (void (QSanSkillButton::*)())(&QSanSkillButton::skill_activated), select, &SelectBox::popup);
            connect(btn, (void (QSanSkillButton::*)())(&QSanSkillButton::skill_deactivated), select, &SelectBox::clear);
            disconnect(btn, (void (QSanSkillButton::*)())(&QSanSkillButton::skill_activated), this, &RoomScene::onSkillActivated);
            connect(select, &SelectBox::onButtonClick, this, &RoomScene::onSkillActivated);
        }
    }
...
void RoomScene::onSkillActivated()
{
    ...
    const ViewAsSkill *vs_skill = NULL;
    const Skill *skill = NULL;
    if (button) {
        vs_skill = button->getViewAsSkill();
        skill = button->getSkill();
    } else {
        SelectBox *select = qobject_cast<SelectBox *>(sender());
        if (select) {
            vs_skill = Sanguosha->getViewAsSkill(select->getSkillName());
            skill = Sanguosha->getSkill(select->getSkillName());
        }
    }

    if (vs_skill && !vs_skill->inherits("FilterSkill")) {
        cancel_button->setEnabled(true);
        bool instance_use = vs_skill->inherits("ZeroCardViewAsSkill");
/*
        if (instance_use && skill) {
            QString select_options = skill->getSelectBox();
            if (select_options.startsWith("guhuo_")) {
                QString guhuo_type = select_options.mid(6);
        ...
        if (card && card->targetFixed() && card->isAvailable(Self)) {
            if (!instance_use) {
                ...
                foreach (const Card *c, cards) {
                    if (vs_skill->viewFilter(QList<const Card *>(), c))
                        return;
                }
                instance_use = true;
            }
...
void RoomScene::showPile(const QString &player_name, QList<int> card_ids, const QString &skill_name)
{
    const ClientPlayer *player = ClientInstance->getPlayer(player_name);
...
void RoomScene::detachSkill(const QString &skill_name)
{
    ...
    if (Self != NULL && Self->getMark("ViewAsSkill_" + skill_name + "Effect") > 0) {
        ...
        if (select_items[skill_name] != NULL)
            select_items[skill_name]->deleteLater(); //for SelectBox
    }
}
