...
void CardItem::goBack(bool playAnimation, bool doFade, AnimationType animation_type)
{
    if (playAnimation) {
        getGoBackAnimation(doFade, animation_type);
        if (m_currentAnimation != NULL)
...
void CardItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    if (m_card) {
		if (_m_validate_suit == Card::SuitToBeDecided) {
