...
bool Dashboard::_addCardItems(QList<CardItem *> &card_items, const CardsMoveStruct &moveInfo)
{
    ...
    if (place == Player::PlaceHand) {
        if (from_place == Player::DrawPile || from_place == Player::DiscardPile || from_place == Player::PlaceTable
...
void Dashboard::skillButtonActivated()
{
    ...
    foreach (QSanSkillButton *btn, _m_skillDock->getAllSkillButtons()) {
        ...
        if (btn->getViewAsSkill() != NULL && btn->isDown()) {
            ...
            QString skill_name = btn->getSkill()->objectName();
            RoomSceneInstance->clearDashboardBox(skill_name);
        }
    }
...
void Dashboard::adjustCards(QList<CardItem *> &cards, bool playAnimation, CardItem::AnimationType animation_type)
{
    _adjustCards();
	foreach(CardItem *card, m_handCards)
        card->goBack(playAnimation, true, cards.contains(card) ? animation_type : CardItem::Normal);
}
...
void Dashboard::_adjustCards()
{
    ...
    _m_highestZ = 0;
    _disperseCards(m_handCards, rowRect, Qt::AlignLeft, true, true);
...
void Dashboard::expandPileCards(const QString &pile_name, bool prepend)
{
	...
	adjustCards(card_items, true, (prepend ? CardItem::FromLeft : CardItem::FromRight));
    update();
...
void Dashboard::updateHandPile(const QString &pile_name, bool add, QList<int> card_ids)
{
	...
	if (add) {
		...
		adjustCards(card_items, true, CardItem::FromLeft);
	} else {
		CardItem *card_item;
		foreach (int card_id, card_ids) {
			if (!pile.contains(card_id)) continue;
...
void Dashboard::onCardItemHover()
{
    ...
    if (card_item) {
        ...
        _m_highestZ = 0;
        _disperseCards(m_handCards, rowRect, Qt::AlignLeft, true, true, m_handCards.indexOf(card_item));
