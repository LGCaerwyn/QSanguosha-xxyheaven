...
sgs.ai_skill_use["@@moshi"] = function(self, prompt)
    local moshi_card = self.player:property("moshi"):toString()
    local use_card = sgs.Sanguosha:cloneCard(moshi_card)
    use_card:setSkillName("moshi")
    if use_card then
...
sgs.ai_skill_invoke.xiemu = true
...
sgs.ai_skill_invoke.junbing_invoke = function(self, data)
...
function sgs.ai_cardsview_valuable.cihuai(self, class_name, player)
    if self.player:getMark("@cihuai") > 0 then
    if class_name == "Slash" then
        return "slash:_cihuai[no_suit:0]=."
    end
    end
end
...
sgs.ai_skill_playerchosen.qizhi = function(self, targets)
    ...
    for _, friend in ipairs(self.friends_noself) do
        ...
        if (self:hasSkills(sgs.lose_equip_skill, friend) and not friend:getEquips():isEmpty())
        or (friend:hasSkills("tuntian+zaoxian"))
        or (self:needToThrowArmor(friend))
        or (friend:hasSkills("lianying|noslianying|sijian") and friend:getHandcardNum() == 1)
        or (friend:hasSkills("shangshi") and friend:getHandcardNum() <= friend:getLostHp()) then
            return friend
        end
    end
    for _, enemy in ipairs(self.enemies) do
        if (not enemy:isNude() and (enemy:isKongcheng() and (enemy:hasSkill("kongcheng") or (enemy:hasSkill("zhiji") and enemy:getMark("zhiji") == 0))))
        or (self:getDangerousCard(enemy) or self:getValuableCard(enemy)) then
            return enemy
        end
    end
    if not self.player:hasSkill("jinqu") then return nil end
    for _, friend in ipairs(self.friends) do
        if not friend:hasSkill("manjuan") then
            return self.player or friend
        end
    end
    return nil
end
